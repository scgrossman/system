                ...
                <ul className={styles.mobile_nav_link_list}>
                    {Object.keys(nav).length !== 0 &&
                        nav.left.map(left_item => {
                            ...
                            const isMoreSports = left_item.title === 'More Sports'
                            ...
                            return (
                                <React.Fragment key={left_item.ID}>
                                    <li className={`${styles.mobile_nav_link_list_item} ${isMoreSports ? styles.more_sports : ''}`}>
                                        ...
                                        {isMoreSports && (
                                            <div onClick={this.toggleMobileMoreSportsMenu}>
                                                <img className={styles.mobile_nav_primary_logo} src={left_item.logo} />
                                                <div className={styles.mobile_nav_primary_title}>{left_item.title}</div>
                                            </div>
                                        )}
                                    </li>
                                    {mobile_more_sports_menu_open &&
                                        isMoreSports &&
                                        left_item.child_menu.length !== 0 && (
                                            <div className={`${styles.mobile_nav_secondary_list} ${
                                                mobile_more_sports_menu_open ? styles.slideIn : styles.slideOut
                                            }`}>
                                                <div className={`${styles.mobile_nav_secondary_back} ${styles.Navigation_chevron}`} onClick={this.toggleMobileMoreSportsMenu}>Back</div>
                ...
                <ul className={styles.mobile_nav_link_list}>
                    {Object.keys(nav).length !== 0 &&
                        nav.right.map(right_item => {
                            ...
                            const isMore = right_item.title === 'More'
                            const doReactPage = isReactUrl(url_formatted)
                            const proper_url = getReactWpUrl(url_formatted, doReactPage)
